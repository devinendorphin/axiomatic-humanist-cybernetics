...
- **[`.github/workflows/verify.yml`](.github/workflows/verify.yml)** — CI: rebuilds every proof on each push, fails on any `sorry`, on `Classical.choice`, on any unexpected axiom, or on a deviation from the published audit footprint (102/46).
...
The build re-checks every proof and prints the axiom audit. Expected: 102 audited theorems, every one at most `[propext, Quot.sound]`, never `Classical.choice`, 46 axiom-free.
