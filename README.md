...
- **[`.github/workflows/verify.yml`](.github/workflows/verify.yml)** — CI: rebuilds every proof on each push, fails on any `sorry`, on `Classical.choice`, on any unexpected axiom, or on a deviation from the published audit footprint (93/43).
...
The build re-checks every proof and prints the axiom audit. Expected: 93 audited theorems, every one at most `[propext, Quot.sound]`, never `Classical.choice`, 43 axiom-free.
