...
**Kernel version 0.13 — 2026-07-14**
...
imports.
**131 theorems · 5 modules + a cross-module bridge · Lean 4.15.0, core only · zero unproven claims.**
...
is robust against MALFORMED inputs; this version opens a new seam class the
review surface had not previously named. **Module 5 — the Seam Ledger
(Contested Attestation Ledger)** addresses the kernel's exposure to
...
Lean — and proved 131 statements about them. A proof assistant does not test a
...
the 131 properties below. **The proofs verify the specification, not the
...
kernel axioms), and **60 of 131 theorems depend on no axioms at all**.
`decCompliant` is a verified `Decidable` instance, checked by elaboration and
additional to the 131.
...
v0.13 ships the wrappers additively alongside the naked `EpInput` /
`Envelope` / `Event` interfaces, which are marked deprecated with a removal
target of **v0.14**.
...
build; a placeholder surfaces as the `sorryAx` axiom. **Expected:** 131
...
this check on every change and fails on any deviation from the 131/60
footprint.
...
deprecation: is v0.14 the right horizon to gate the naked input interfaces
behind the wrapper, given a preserved bypass is technical debt made
normative?
