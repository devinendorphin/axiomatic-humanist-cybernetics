...
**Kernel version 0.5 — 2026-07-12**

Companion artifact to AHC v3.1 and Companion A (integrated).
**81 theorems · 4 modules · Lean 4.15.0, core only · zero unproven claims.**
Source digest (sha256 over sorted sources):
`a49ec2bc8f4a1490679a1842e3a2dee85c0594277aca621da4a63c23d750e5a9`
...
five rulings made on questions the first two review rounds surfaced — and
...
Lean — and proved 81 statements about them. A proof assistant does not test a
rule on examples; it verifies that no counterexample can exist, anywhere,
ever, within the rules as written.

This is the fifth iteration of that kernel. The first (v0.1, 34 theorems) was
...
understated — and *what evidence would prove the order wrong.*
...
limited: the specification, as modeled, provably possesses the 81 properties
...
kernel axioms), and **36 of 81 theorems depend on no axioms at all**, including
...
### F. Modeling disclosures — the contestation targets
...
### G. What is not verified, and why
...
a placeholder surfaces as the `sorryAx` axiom. **Expected:** 81 audited
theorems, each at most `[propext, Quot.sound]`; never `Classical.choice`; 36
...
any deviation from the 81/36 footprint.
...
**F-4 (Strengthening candidates).** Which currently-unverified sentences are
order-theoretic enough to join the kernel? Two named candidates: deriving the
episode machine's `exceedance` input from Module 3's ATG threshold machinery
(closing the last free input in the emergency loop), and later-register
(civic/technical) compliance obligations checked at ship time (T+72h, T+30d),
paralleling the T+0 disclosures now proved.
